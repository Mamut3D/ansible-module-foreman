...
def domains_equal(data, domain, compareable_keys):
    if not all(data.get(key, None) == domain.get(key, None) for key in compareable_keys):
        return False
...
def ensure(module):
    compareable_keys = ['name', 'fullname']
    ...
    if domain:
        ...
        if not domains_equal(data, domain, compareable_keys):
            try:
