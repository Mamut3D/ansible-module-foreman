...
def subnets_equal(data, subnet, compareable_keys):
    if not all(data.get(key, None) == subnet.get(key, None) for key in compareable_keys):
        return False
...
def ensure(module):
    compareable_keys = ['name', 'dns_primary', 'dns_secondary', 'gateway', 'ipam', 'boot_mode', 'mask', 'network',
                        'vlanid', 'from', 'to']
    ...
    if subnet:
        ...
        if not subnets_equal(data, subnet, compareable_keys):
            try:
