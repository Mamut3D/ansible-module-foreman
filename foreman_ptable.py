...
def ensure():
    ...
    if ptable and state == 'absent':
        try:
            ptable = theforeman.delete_partition_table(id=ptable['id'])
        except ForemanError as e:
    ...
    if ptable and state == 'present':
        try:
            ptable = theforeman.get_partition_table(id=ptable['id'])
        except ForemanError as e:
            module.fail_json(msg='Could not get partition table to update: {0}'.format(e.message))
        if not ptables_equal(data, ptable):
            try:
                ptable = theforeman.update_partition_table(id=ptable['id'], data=data)
            except ForemanError as e:
